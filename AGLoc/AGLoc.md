# Robust Lifelong Indoor LiDAR Localization Using the Area Graph (AGLoc)
...
이를 위해 AGLoc은 방·복도를 polygon으로 표현하는 Area Graph를 사용한다. 각 공간은 area node로 표현되고, 인접한 공간을 연결하는 문이나 통로는 passage로 표현된다. 또한 건물–층–방의 계층 구조와 층 높이, 공간 종류 등의 semantic information을 저장할 수 있다.
...
1. **B. 2D Clutter-Free Point Set**을 생성한다.
2. **C. Intersection and Signed Distance Calculation**으로 현재 pose에서 point-to-line residual을 구한다.
3. **E. Weight Function**으로 각 residual의 신뢰도를 계산한다.
4. **G. Corridorness Downsampling**으로 한 방향의 wall point가 과도하게 많은 경우 dominant orientation의 point를 추가로 줄인다.
5. **F. Weighted Point-to-Line ICP**로 weighted residual을 최소화하도록 pose를 보정한다.
...
WiFi localization이 제공한 약 6 m 반경의 탐색 범위에서 여러 \((x,y,\mathrm{yaw})\) pose 후보를 균일하게 생성한다.
...
\sum_{j=1}^{M}
\begin{cases}
\lvert sd_j\rvert, & \lvert sd_j\rvert<d\\
2, & \text{otherwise}
\end{cases}
$$
...
<img width="241" height="66" alt="image" src="https://github.com/user-attachments/assets/1c76502c-9aec-4d14-8d78-a3fe980acc4d" />
...
\begin{cases}
0, & sd_j\le-1\\[4pt]
\dfrac{1}{1.5\lvert sd_j\rvert+1}, & -1<sd_j\le0\\[8pt]
\dfrac{1}{3sd_j+1}, & 0<sd_j<3\\[8pt]
0, & sd_j\ge3
\end{cases}
$$

- \(sd_j\le-1\): wall보다 1 m 이상 안쪽에 있어 대형 가구 등의 clutter일 가능성이 높은 point
- \(-1<sd_j\le0\): wall 안쪽에 있지만 wall에 가까울수록 높은 weight 부여
- \(0<sd_j<3\): wall 바깥쪽으로 벗어났으며 거리가 커질수록 weight 감소
- \(sd_j\ge3\): 반사 또는 이상치 가능성이 높은 point
...
여기서 \(\pi(p_j,R,t)\)는 pose \((R,t)\)로 변환한 LiDAR point와 대응 Area Graph polygon segment 사이의 signed point-to-line distance를 의미한다.
...
이를 완화하기 위해 현재 scan point들이 대응된 Area Graph polygon segment의 orientation을 \(5^\circ\) 간격의 histogram에 누적한다. 선분 orientation은 \(180^\circ\) 주기를 가지므로 총 36개의 bin을 사용한다.
...
- \(N_{\max}\): point가 가장 많이 누적된 orientation bin의 point 수
- \(N_m\): 전체 대응 point 수
- \(Cor\)가 클수록 한 방향의 wall이 관측을 지배함
- 두 직교 방향이 비슷하게 관측되면 \(Cor\approx0.5\)
- 한 방향의 wall만 관측되면 \(Cor=1\)
