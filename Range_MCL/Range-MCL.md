...
- 게재처: - 게재처: 2021 IEEE International Conference on Robotics and Automation (ICRA)
- 연도: 2021
...
시스템은 Monte Carlo Localization(MCL)을 기반으로 한다. 
또한 triangular mesh map과 OpenGL 기반 렌더링, tile map 구조를 사용하여 수렴 이후 LiDAR sensor frame rate에서 online pose tracking이 가능함을 보였다.
...
현재 LiDAR scan의 point cloud를 spherical projection을 통해 range image로 변환한다. 각 LiDAR point \(p_i=(x,y,z)\)에 대해 range

\[
r = \|p\|_2
\]

를 계산하고, 각각 수평/수직 angle에 해당하는 image coordinate \((u,v)\)로 투영한다. 이렇게 생성된 range image는 이후 observation model에서 particle별 synthetic range image와 비교되는 실제 관측값 \(z_t\)로 사용된다.
...
본 논문은 환경의 사전 map \(M\)을 triangular mesh로 표현한다. 
Triangular mesh는 대규모 point cloud map보다 compact하며, particle pose에서 synthetic range image를 빠르게 렌더링할 수 있다.
...
\[
d = \frac{1}{N}\sum |z_t - z^j|
\]

여기서 \(N\)은 현재 range image에서 valid pixel의 수이다. 즉, 실제 LiDAR range 값이 존재하는 유효한 픽셀들에 대해, real range image와 synthetic range image의 같은 pixel 위치끼리 range 차이를 계산하고 평균낸다. 차이가 작을수록 해당 particle pose에서 map을 보았을 때 실제 LiDAR 관측과 잘 일치한다는 뜻이므로 likelihood와 particle weight가 높아진다.
