...
문제 정식화: 세션 $S$는 pose graph $\mathcal{G}=(\mathcal{V},\mathcal{E})$와 keyframe DB $\mathcal{F}$를 가진다. 초기 pose $T^b_{init}$ 확보 후, 이후 추정은 joint factor graph $\{\mathcal{G}^a,\mathcal{G}^b\}$의 MAP 문제다:
